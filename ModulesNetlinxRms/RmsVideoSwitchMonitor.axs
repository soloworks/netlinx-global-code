MODULE_NAME='RmsVideoSwitchMonitor'(DEV vdvRMS,
                                           DEV vdvDevice)
...
CHAR MONITOR_ASSET_NAME[50]             = 'Video Switch';
...
DEFINE_FUNCTION SynchronizeAssetParameters()
{
  ...
  //Synchronize all snapi HAS_xyz components
   SynchronizeAssetParametersSnapiComponents(assetClientKey)
	...
	// Input State
	RmsAssetParameterEnqueueSetValue(assetClientKey,'asset.input',  MONITOR_ASSET_INPUT)
...
DEFINE_FUNCTION RegisterAssetControlMethods()
{
  ...
  //RegisterAssetControlMethodsSnapiComponents(assetClientKey);
	// Input Select (From VGA|RGB|DVI|HDMI|VID1|VID2|SVID|DVD/HD1|OPTION|DVD/HD2|DPORT|HDMI2)
  RmsAssetControlMethodEnqueue        (assetClientKey,  'asset.customaction.input', 'Current Input', 'Set Input');
			...
			CASE 'PROPERTY-':{
				SWITCH(REMOVE_STRING(DATA.TEXT,',',1)){
					...
					CASE 'STATE,':{
						SWITCH(REMOVE_STRING(DATA.TEXT,',',1)){
							CASE 'INPUT,':{
								MONITOR_ASSET_INPUT = DATA.TEXT
								IF(IsRmsReady()){ RmsAssetParameterSetValue(assetClientKey,'asset.input',MONITOR_ASSET_INPUT); }
							}
							CASE 'TEMP,':{
								MONITOR_PROP_TEMP = ATOI(DATA.TEXT)
								IF(IsRmsReady()){ RmsAssetParameterSetValueLevel(assetClientKey,'asset.temp',MONITOR_PROP_TEMP); }
							}
							CASE 'NET_IP,':{
								MONITOR_ASSET_IP_ADDRESS = DATA.TEXT
								IF(IsRmsReady()){ RmsAssetParameterSetValue(assetClientKey,'asset.network.ip',MONITOR_ASSET_IP_ADDRESS); }
							}
						}
					}
				}
