MODULE_NAME='mExtronDMP44LC'(DEV vdvControl,DEV vdvObjects[],DEV dvDevice)
...
DEFINE_TYPE STRUCTURE uObject{
	...
	SINTEGER GAIN_VALUE_RAW		// Current Value (Raw)
	SINTEGER GAIN_LIMITS_RAW[2]// Object Limits (Raw)
	FLOAT		GAIN_VALUE_dB		// Current Value (dB)
	FLOAT		GAIN_LIMITS_dB[2]	// Object Limits (dB)
				...
				ACTIVE(LEFT_STRING(pFB,3) == 'DsG'):{
					...
					FOR(o=1; o<= LENGTH_ARRAY(vdvObjects); o++){
						IF(myDMP.OBJECTS[o].ADDRESS == ADDRESS){
							myDMP.OBJECTS[o].GAIN_VALUE_RAW = ATOI(pFB)
							myDMP.OBJECTS[o].GAIN_VALUE_dB = myDMP.OBJECTS[o].GAIN_VALUE_RAW / 10
						}
					}
				}
		...
		SWITCH(fnStripCharsRight(REMOVE_STRING(DATA.TEXT,'-',1),1)){
				...
				SWITCH(fnStripCharsRight(REMOVE_STRING(DATA.TEXT,',',1),1)){
					...
					CASE 'ID':{
						...
						SWITCH(myDMP.OBJECTS[o].TYPE){
							...
							CASE OBJ_TYPE_FADER:{
								SELECT{
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 30100):{
										// Is a Input Gain Control
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] =  -18.0
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =   24.0
									}
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 30200):{
										// Is a Input Gain Control
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] =  -100
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =     6
									}
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 40100):{
										// Is a Input Gain Control
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] =  -18.0
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =   80.0
									}
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 50000):{
										// Is a Pre-Mixer Gain
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] = -100.0
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =   12.0
									}
									...
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 60100):{
										...
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] = -100.0
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =    0.0
									}
									ACTIVE(myDMP.OBJECTS[o].ADDRESS < 70000):{
										// Is a Post Mixer Trim
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] =  -12.0
										myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] =   12.0
									}
								}

								// Set Ranges
								SEND_STRING DATA.DEVICE, "'RANGE-',ITOA(myDMP.OBJECTS[o].GAIN_LIMITS_dB[1]),',',ITOA(myDMP.OBJECTS[o].GAIN_LIMITS_dB[2]),''"

								// Set Normalised Limits
								myDMP.OBJECTS[o].GAIN_LIMITS_RAW[1] = TYPE_CAST(myDMP.OBJECTS[o].GAIN_LIMITS_dB[1] * 10)
								myDMP.OBJECTS[o].GAIN_LIMITS_RAW[2] = TYPE_CAST(myDMP.OBJECTS[o].GAIN_LIMITS_dB[2] * 10)
							}
						}
					}
				}
			...
			CASE 'VOLUME':{
				SWITCH(DATA.TEXT){
					...
					DEFAULT:{
						myDMP.OBJECTS[o].NEW_GAIN_VALUE = ATOI(DATA.TEXT) * 10
...
TIMELINE_EVENT[TLID_VOL_10]{
	...
	IF(myDMP.OBJECTS[o].NEW_GAIN_PEND){
		SWITCH(myDMP.OBJECTS[o].TYPE){
			CASE OBJ_TYPE_GROUP:fnAddToQueue("$1B,'D',ITOA(myDMP.OBJECTS[o].ADDRESS),'*',ITOA(myDMP.OBJECTS[o].NEW_GAIN_VALUE),'GRPM',$0D",FALSE)
			CASE OBJ_TYPE_FADER:fnAddToQueue("$1B,'G',ITOA(myDMP.OBJECTS[o].ADDRESS),'*',ITOA(myDMP.OBJECTS[o].NEW_GAIN_VALUE),'AU',$0D",FALSE)
		}
